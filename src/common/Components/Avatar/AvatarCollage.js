      ...
      if (participant) {
        ...
        participant.avatar_960) {
          return (
            ...
            participant.avatar_384 ||
            participant.avatar_960})`, backgroundRepeat: "no-repeat", zIndex: -index+10, borderColor: "white", borderStyle: "solid"}} className="default-message-member-avatar-icon" />
          )
        }
        if (participant.email) {
          return (
            <div key={index} className="default-message-member-avatar-icon" style={{zIndex: -index+10}}>
        ...
        if (participant.first_name && participant.last_name) {
          return (
            <div key={index} className="default-message-member-avatar-icon" style={{zIndex: -index+10, borderRadius: "50%", borderColor: "white", borderStyle: "solid", backgroundColor: widget ? widget.initialConfig.keyColor : chatpage ? chatpage.initialConfig.keyColor : "#f7a444"}}>
  ...
  constructChannelAvatar() {
    ...
    if (widget) {
      if (widget.initialConfig.channel.avatar) {
        return (
          <div style={{borderRadius: "50%", borderColor: "white", borderStyle: "solid", width: "43px", height: "43px", backgroundImage: `url(${widget.initialConfig.channel.avatarUrl})`}}></div>
        )
      }
    }
    if (chatpage) {
      if (chatpage.initialConfig.channel.avatar) {
        return (
          <div style={{borderRadius: "50%", borderColor: "white", borderStyle: "solid", width: "43px", height: "43px", backgroundImage: `url(${chatpage.initialConfig.channel.avatarUrl})`}}></div>
        )
      }
    }
  }
